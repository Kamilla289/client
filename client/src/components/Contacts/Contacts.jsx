...
const Contacts = () => {
  ...
  const handleSubmit = async () => {
    const nameValue = inputSubmit[1] || '';
    const emailValue = inputSubmit[2] || '';
      ...
      <Sector noHeight>
        <div className="contact-block-mraz">
          ...
          <div className="contacts-block">
            <div className="contacts-block-input">
                ...
                <div key={data.index} className="contact-frame">
                  ...
                  {data.index === 3 ? (
                    <IMaskInput
                      ...
                      value={inputSubmit[data.index] || ''}
                      unmask={false}
                      prepare={(str) => {
                        // Если вставляют номер с 8 в начале, заменяем на 7
                        if (str.startsWith('8')) {
                          return '7' + str.slice(1);
                        }
                        // Если вставляют сразу без +7, тоже исправим
                        if (str.startsWith('7')) {
                          return str; // уже ок
                        }
                        return str;
                    ...
                    <input
                      value={inputSubmit[data.index] || ''}
                      onChange={(e) => handleInputSubmit(e, data.index)}
                      id={data.id}
                      className="input-form"
                      type={data.type}
                      placeholder={data.placeholder}
              ...
              <div className="policy">
                <label className="custom-checkbox">
                  ...
                  <span className="checkbox-box">
                    ...
                    >
                      <path
                        d="M1.50098 13.9995L13.6206 29.5818C14.5068 30.7213 16.2715 30.5752 16.9583 29.3056L32.001 1.49951"
                        stroke={theme === 'dark' ? '#a500bb' : '#B6FF5C'}
                        strokeWidth="3"
                        strokeLinecap="round"
                      />
                    </svg>
                ...
                <div className="text-policy">
                  Согласие на <BasicModal />
                </div>
              </div>
            </div>
            <button onClick={handleSubmit} disabled={!isChecked} className="button-gradient">
              Отправить
            </button>
          </div>
        </div>
